...
class TutApp {
    ...
    getSupportedMimeType() {
        const mimeTypes = [
            ...
            'video/mp4;codecs=avc1.58A01E', // H.264 High
            'video/mp4', // 通用MP4
    ...
    updateFormatDisplay() {
        ...
        const formatName = formatInfo[this.supportedMimeType] || this.supportedMimeType;
        const isMP4 = this.supportedMimeType.includes('mp4');

        formatDisplay.textContent = `${formatName} ${isMP4 ? '✅' : '⚠️'}`;
        formatDisplay.style.color = isMP4 ? '#34c759' : '#ff9500';
    }
    ...
    startRecording() {
        ...
        console.log('录制格式:', this.supportedMimeType);
    ...
    saveVideo() {
        ...
        a.href = url;
        a.download = `tut_recording_${Date.now()}${fileExtension}`;
        ...
        if (fileExtension === '.mp4') {
            alert('视频已下载！MP4格式可以直接保存到iOS相册。');
        } else {
            alert('视频已下载！WebM格式，您可以在文件应用中查看。');
        }
    }
