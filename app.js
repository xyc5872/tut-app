...
class TutApp {
    ...
    checkRealMovement() {
        ...
        // 计算参考锚点1的移动向量
        const ref1MoveX = refAnchor1.trackingPosition.x - refAnchor1.x;
        const ref1MoveY = refAnchor1.trackingPosition.y - refAnchor1.y;

        // 计算参考锚点2的移动向量
        const ref2MoveX = refAnchor2.trackingPosition.x - refAnchor2.x;
        const ref2MoveY = refAnchor2.trackingPosition.y - refAnchor2.y;

        // 计算主锚点的移动向量
        const mainMoveX = mainAnchor.trackingPosition.x - mainAnchor.x;
        const mainMoveY = mainAnchor.trackingPosition.y - mainAnchor.y;

        // 计算全局移动向量（两个参考锚点的平均移动）
        const globalMoveX = (ref1MoveX + ref2MoveX) / 2;
        const globalMoveY = (ref1MoveY + ref2MoveY) / 2;
