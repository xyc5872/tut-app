...
class TutApp {
    ...
    setupMultiAnchors(mainX, mainY) {
        ...
        // 设置四个角落的参考锚点（距离边缘100像素，确保在静态背景区域）
        ...
        this.anchors.push({
            x: cornerOffset,
            y: cornerOffset,
            trackingPosition: { x: cornerOffset, y: cornerOffset },
            initialImageData: null,
            isReference: true
        });
        ...
        // 参考锚点2：右上角
        this.anchors.push({
            x: maxWidth - cornerOffset,
            y: cornerOffset,
            trackingPosition: { x: maxWidth - cornerOffset, y: cornerOffset },
            initialImageData: null,
            isReference: true
        });
        ...
        this.anchors.push({
            x: cornerOffset,
            y: maxHeight - cornerOffset,
            trackingPosition: { x: cornerOffset, y: maxHeight - cornerOffset },
            initialImageData: null,
            isReference: true
        });
        ...
        this.anchors.push({
            x: maxWidth - cornerOffset,
            y: maxHeight - cornerOffset,
            trackingPosition: { x: maxWidth - cornerOffset, y: maxHeight - cornerOffset },
            initialImageData: null,
            isReference: true
        });
        ...
        console.log('已设置5个锚点：1个主锚点 + 4个四角参考锚点');
    }
    ...
    // 使用块匹配算法追踪所有锚点的移动
    trackMovement() {
        if (this.anchors.length < 5 || !this.stream) {
    ...
    // 检查主锚点是否真正移动（使用移动向量补偿法）
    checkRealMovement() {
        if (this.anchors.length < 5) return false;

        const mainAnchor = this.anchors[this.mainAnchorIndex];

        // 计算主锚点的移动向量
        const mainMoveX = mainAnchor.trackingPosition.x - mainAnchor.x;
        const mainMoveY = mainAnchor.trackingPosition.y - mainAnchor.y;
        ...
        for (let i = 1; i < this.anchors.length; i++) {
            const refAnchor = this.anchors[i];
            totalRefMoveX += refAnchor.trackingPosition.x - refAnchor.x;
            totalRefMoveY += refAnchor.trackingPosition.y - refAnchor.y;
        }

        // 计算全局移动向量（四个参考锚点的平均移动）
        const globalMoveX = totalRefMoveX / 4;
        const globalMoveY = totalRefMoveY / 4;

        // 计算主锚点的相对移动（减去全局移动）
        const relativeMoveX = mainMoveX - globalMoveX;
        const relativeMoveY = mainMoveY - globalMoveY;

        // 计算相对移动距离
        const relativeMoveDistance = Math.sqrt(relativeMoveX * relativeMoveX + relativeMoveY * relativeMoveY);

        // 根据灵敏度判断是否移动
        const threshold = (100 - this.settings.sensitivity) * 0.8; // 相对移动阈值

        const hasMoved = relativeMoveDistance > threshold;

        // 调试输出
        if (hasMoved) {
            console.log(`检测到真移动！相对距离: ${relativeMoveDistance.toFixed(2)}, 阈值: ${threshold.toFixed(2)}`);
        }
